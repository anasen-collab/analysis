...
void SiPulser_All(void)
{
  ...
  for (Int_t MBID=1; MBID<3; MBID++)
	...
	{ 
	  ////for the front of the detectors //run645 - positive polarity - fronts
	  //if((CBID==1 || CBID==2 || CBID==5 || CBID==6 || CBID==9 ||CBID==10) ||(MBID==1 && (CBID==11 || CBID==12 || CBID==13 || CBID==14))){
	 

	    ////for the back of the detectors //run643 - negative polarity - backs
		...
		if (MBID==1 && CBID==1 && ChNum ==14){ 
		  continue;
		}	      
		if(MBID==1 && CBID==8 && (ChNum==0 || ChNum==15)){
		  continue;
		} 
		if(MBID ==1 && CBID ==12  && ChNum ==6){
		  continue;
		}
		if(MBID ==2 && CBID ==7  && (ChNum ==4 || ChNum ==5 || ChNum ==9 || ChNum ==10)){
		  continue;
		}
		if(MBID ==2 && CBID ==12  && (ChNum ==13 || ChNum ==14)){
		  continue;
		}
