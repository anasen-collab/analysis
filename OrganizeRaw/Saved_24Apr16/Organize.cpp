...
int main(int argc, char* argv[]){
  ...
  MainTree->Branch("Si.NHitsInDet",&Si.NHitsInDet);
  MainTree->Branch("Si.DetID",&Si.DetID);
  MainTree->Branch("Si.UpMult",&Si.UpMult);
  MainTree->Branch("Si.DownMult",&Si.DownMult);
  MainTree->Branch("Si.BackMult",&Si.BackMult);
  MainTree->Branch("Si.UpChNum",&Si.UpChNum);
  MainTree->Branch("Si.DownChNum",&Si.DownChNum);
  MainTree->Branch("Si.BackChNum",&Si.BackChNum);
  MainTree->Branch("Si.EnergyUp_Raw",&Si.EnergyUp_Raw);
  MainTree->Branch("Si.EnergyDown_Raw",&Si.EnergyDown_Raw);
  MainTree->Branch("Si.EnergyBack_Raw",&Si.EnergyBack_Raw);
  MainTree->Branch("Si.EnergyUp_Pulser",&Si.EnergyUp_Pulser);
  MainTree->Branch("Si.EnergyDown_Pulser",&Si.EnergyDown_Pulser);
  MainTree->Branch("Si.EnergyBack_Pulser",&Si.EnergyBack_Pulser);
  MainTree->Branch("Si.EnergyUp_Rel",&Si.EnergyUp_Rel);
  MainTree->Branch("Si.EnergyDown_Rel",&Si.EnergyDown_Rel);
  MainTree->Branch("Si.EnergyBack_Rel",&Si.EnergyBack_Rel);
  MainTree->Branch("Si.EnergyUp_Cal",&Si.EnergyUp_Cal);
  MainTree->Branch("Si.EnergyDown_Cal",&Si.EnergyDown_Cal);
  MainTree->Branch("Si.EnergyBack_Cal",&Si.EnergyBack_Cal);
  MainTree->Branch("Si.TimeUp",&Si.TimeUp);
  MainTree->Branch("Si.TimeDown",&Si.TimeDown);
  MainTree->Branch("Si.TimeBack",&Si.TimeBack);

  MainTree->Branch("Si.HitType",&Si.HitType);
  MainTree->Branch("Si.Energy",&Si.Energy);
  MainTree->Branch("Si.Time",&Si.Time);
  MainTree->Branch("Si.X",&Si.X);
  MainTree->Branch("Si.Y",&Si.Y);
  MainTree->Branch("Si.Z",&Si.Z);
  MainTree->Branch("Si.XW",&Si.XW);
  MainTree->Branch("Si.YW",&Si.YW);
  MainTree->Branch("Si.ZW",&Si.ZW);
  MainTree->Branch("Si.RW",&Si.RW);
  MainTree->Branch("Si.PhiW",&Si.PhiW);
  MainTree->Branch("Si.RFSubtract",&Si.RFSubtract);
  ...
  while (!inFileList.eof()){//loop over all of the incoming root files----------------------------------------------------------------------------------------------
    ...
    vector<Int_t> X3ChUp, X3ChDown, X3ChBack;
    vector<Int_t> QQQChUp, QQQChBack;

    vector<Int_t> DetID_row, HitType_row;

    vector<Double_t> X3EnergyUp_Raw;
    vector<Double_t> X3EnergyDown_Raw;
    vector<Double_t> X3EnergyBack_Raw;

    vector<Double_t> QQQEnergyUp_Raw;
    vector<Double_t> QQQEnergyBack_Raw;

    vector<Double_t> X3EnergyUp_Pulser;
    vector<Double_t> X3EnergyDown_Pulser;
    vector<Double_t> X3EnergyBack_Pulser;

    vector<Double_t> QQQEnergyUp_Pulser;
    vector<Double_t> QQQEnergyBack_Pulser;

    vector<Double_t> X3EnergyUp_Rel;
    vector<Double_t> X3EnergyDown_Rel;
    vector<Double_t> X3EnergyBack_Rel;

    vector<Double_t> QQQEnergyUp_Rel;
    vector<Double_t> QQQEnergyBack_Rel;

    vector<Double_t> X3EnergyUp_Cal;
    vector<Double_t> X3EnergyDown_Cal;
    vector<Double_t> X3EnergyBack_Cal;

    vector<Double_t> QQQEnergyUp_Cal;
    vector<Double_t> QQQEnergyBack_Cal;

    vector<Double_t> X3TimeUp, X3TimeDown, X3TimeBack;
    vector<Double_t> QQQTimeUp, QQQTimeBack;

    vector<Double_t> X3Energy_row, QQQEnergy_row;
    vector<Double_t> X3Time_row, QQQTime_row;

    vector<Double_t> RFSubtract_row;

    vector<Double_t> x_row, y_row, z_row;
    vector<Double_t> xw_row, yw_row, zw_row;

    vector<Double_t> rw_row, phiw_row;
	...
	for (int j=0; j<MaxX3Ch; j++){
      ...
      X3EnergyDown_Raw.clear();
      X3EnergyBack_Raw.clear();
      QQQEnergyUp_Raw.clear();
      QQQEnergyBack_Raw.clear();
      X3EnergyUp_Pulser.clear();
      X3EnergyDown_Pulser.clear();
      X3EnergyBack_Pulser.clear();
      QQQEnergyUp_Pulser.clear();
      QQQEnergyBack_Pulser.clear();
      X3EnergyUp_Rel.clear();
      X3EnergyDown_Rel.clear();
      X3EnergyBack_Rel.clear();
      QQQEnergyUp_Rel.clear();
      QQQEnergyBack_Rel.clear();
      X3EnergyUp_Cal.clear();
      X3EnergyDown_Cal.clear();
      X3EnergyBack_Cal.clear();
      QQQEnergyUp_Cal.clear();
      QQQEnergyBack_Cal.clear();
	...
	for (int j=0; j<MaxX3Ch; j++){
	  if ( (SiEnergy_Cal[i][j] > 0.0)){
	    if (j<4){
	      X3ChBack.push_back(j);
	      X3EnergyBack_Raw.push_back(SiEnergy[i][j]);
	      X3EnergyBack_Pulser.push_back(SiEnergy_Pulser[i][j]);
	      X3EnergyBack_Rel.push_back(SiEnergy_Rel[i][j]);
	      X3EnergyBack_Cal.push_back(SiEnergy_Cal[i][j]);
	      X3TimeBack.push_back(SiTime[i][j]);
	    }else if(j>3 && j<8){
	      X3ChUp.push_back(j-4);
	      X3EnergyUp_Raw.push_back(SiEnergy[i][j]);
	      X3EnergyUp_Pulser.push_back(SiEnergy_Pulser[i][j]);
	      X3EnergyUp_Rel.push_back(SiEnergy_Rel[i][j]);
	      X3EnergyUp_Cal.push_back(SiEnergy_Cal[i][j]);
	      X3TimeUp.push_back(SiTime[i][j]);
	    }else if(j>7 && j<12){
	      X3ChDown.push_back(j-8);
	      X3EnergyDown_Raw.push_back(SiEnergy[i][j]);
	      X3EnergyDown_Pulser.push_back(SiEnergy_Pulser[i][j]);
	      X3EnergyDown_Rel.push_back(SiEnergy_Rel[i][j]);
	      X3EnergyDown_Cal.push_back(SiEnergy_Cal[i][j]);
	      X3TimeDown.push_back(SiTime[i][j]);
	    }
	  }
	}
	//if ( Si.DownMult[Si.NSiHits]!=0 || Si.UpMult[Si.NSiHits]!=0 || Si.BackMult[Si.NSiHits]!=0 ){
	if ( X3EnergyDown_Cal.size()!=0 || X3EnergyUp_Cal.size()!=0 || X3EnergyBack_Cal.size()!=0 ){
	  Si.UpMult.push_back(X3EnergyUp_Cal.size());
	  Si.DownMult.push_back(X3EnergyDown_Cal.size());
	  Si.BackMult.push_back(X3EnergyBack_Cal.size());
	  DetID_row.push_back(i+4);
	  Si.NHitsInDet.push_back(1);

	  HitType_row.push_back(Si.BackMult[0]*100 + Si.UpMult[0]*10 + Si.DownMult[0]);
	  Si.EnergyUp_Raw.push_back(X3EnergyUp_Raw);
	  if ( HitType_row[0] == 111 ){
	    if ( X3ChDown[0]==X3ChUp[0] ){
	      X3Energy_row.push_back(X3EnergyBack_Cal[0]);
	      X3Time_row.push_back(X3TimeBack[0]);
	      RFSubtract_row.push_back(X3Time_row[0] - RFTime);
	      Double_t FinalZUp=0,FinalZUpCal=-10, FinalZDown=0, FinalZDownCal=-10;
	      FinalZDown = 2*X3EnergyDown_Cal[0]/X3EnergyBack_Cal[0]-1;
	      CMAP->PosCal(DetID_row[0],X3ChDown[0],X3ChBack[0],FinalZDown,FinalZDownCal);
	      FinalZUp = 1-2*X3EnergyUp_Cal[0]/X3EnergyBack_Cal[0];
	      CMAP->PosCal(DetID_row[0],X3ChUp[0],X3ChBack[0],FinalZUp,FinalZUpCal);
	      if (FinalZDownCal > -1 && FinalZUpCal > -1){
		z_row.push_back(FinalZUpCal);
		x_row.push_back(XRandom->Rndm()+(3-X3ChUp[0]));
	      }else{
		HitType_row[0] += 1000;
	      }
	      Double_t xw=0, yw=0, zw=0, rw=0, phiw=0;
	      CMAP->GetX3WorldCoordinates(DetID_row[0],x_row[0],z_row[0],xw,yw,zw,rw,phiw);
	      xw_row.push_back(xw);
	      yw_row.push_back(yw);
	      zw_row.push_back(zw);
	      rw_row.push_back(rw);
	      phiw_row.push_back(phiw);
	    }else{
	      HitType_row[0] += 1000;
	    }
	  }

	  Si.DetID.push_back(DetID_row);
	  Si.HitType.push_back(HitType_row);
	  Si.UpChNum.push_back(X3ChUp);
	  Si.DownChNum.push_back(X3ChDown);
	  Si.BackChNum.push_back(X3ChBack);

	  Si.EnergyUp_Raw.push_back(X3EnergyUp_Raw);
	  Si.EnergyDown_Raw.push_back(X3EnergyDown_Raw);
	  Si.EnergyBack_Raw.push_back(X3EnergyBack_Raw);
	  Si.EnergyUp_Pulser.push_back(X3EnergyUp_Pulser);
	  Si.EnergyDown_Pulser.push_back(X3EnergyDown_Pulser);
	  Si.EnergyBack_Pulser.push_back(X3EnergyBack_Pulser);
	  Si.EnergyUp_Rel.push_back(X3EnergyUp_Rel);
	  Si.EnergyDown_Rel.push_back(X3EnergyDown_Rel);
	  Si.EnergyBack_Rel.push_back(X3EnergyBack_Rel);
	  Si.EnergyUp_Cal.push_back(X3EnergyUp_Cal);
	  Si.EnergyDown_Cal.push_back(X3EnergyDown_Cal);
	  Si.EnergyBack_Cal.push_back(X3EnergyBack_Cal);

	  Si.TimeUp.push_back(X3TimeUp);
	  Si.TimeDown.push_back(X3TimeDown);
	  Si.TimeBack.push_back(X3TimeBack);

	  Si.Energy.push_back(X3Energy_row);
	  Si.Time.push_back(X3Time_row);
	  Si.X.push_back(x_row);
	  Si.Y.push_back(y_row);
	  Si.Z.push_back(z_row);
	  Si.XW.push_back(xw_row);
	  Si.YW.push_back(yw_row);
	  Si.ZW.push_back(zw_row);
	  Si.RW.push_back(rw_row);
	  Si.PhiW.push_back(phiw_row);
	  Si.RFSubtract.push_back(RFSubtract_row);
